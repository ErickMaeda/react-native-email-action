...
export const sendEmail = ({ to, subject, body, cancelText = "Cancel" }) => new Promise(async (resolve, reject) => {
            ...
            ActionSheetIOS.showActionSheetWithOptions({
                ...
                if (buttonIndex != (options.length - 1)) {
                    if (options[buttonIndex] == 'Gmail') {
                        link = `googlegmail:///co?subject=${subject}&body=${body}&to=${to}`;
                    } else if (options[buttonIndex] == 'Outlook') {
                        link = `ms-outlook://compose?to=${to}&subject=${subject}&body=${body}`;
                    }
